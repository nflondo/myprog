 ...
 * Purpose:  Uses pointers to type double variables to accept 5 numbers from the user, sort them, and print them
 *so the screen implementing a linked list                        *
 *============================================================*/
#include <stdio.h>
#include <stdlib.h>

/* If NULL not defined to 0, then define it */
#ifndef NULL  
#define NULL 0
#endif
...
struct list
{
  int    x;     /* using an int to hold a char */
...
int main(void)
{
   ...
   while ( i++ < 5 )      /* build a list based on 5 items given */
   {
      ...
      } while( (ch < 'a' || ch > 'z') && (ch < 'A' || ch > 'Z'));  /*or use isalpha() function 
	*/
      first = add_to_list( x, first );
   }
...
LISTPTR add_to_list( int x, LISTPTR first )
{
   ...
   else    /* not first record */
   {
      ...
      else   /* it is being added to the middle or end */
      {
         ...
         else
         {
            ...
            {
               if (new_rec->x < tmp_rec->x ) /* 1 b4 end */
               {
                  new_rec->next_rec = tmp_rec;
                  prev_rec->next_rec = new_rec;
               }
