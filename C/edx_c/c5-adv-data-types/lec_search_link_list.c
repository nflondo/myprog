...
//! heap=showMemory(start=348, cursors=[ptr,start])
    struct digit * ptr = start;
    while ((ptr!=NULL) && (ptr->num!=number)) { // ptr != NULL is evaluated first
                                                // so it prevents searching in a NULL
                                                // element
        ptr = ptr->next;
    }
    // while looop will exit and ptr is either going to be NULL if item was
    // not found or it will have a memory value.
    return(ptr);
}