...
void freeNumber(struct digit *start) {
    ...
    struct digit * ptr = start;
    struct digit * tmp;  // need this to save values before deleting them
    while (ptr!=NULL) {
        tmp = ptr->next; // tmp gets next element in list
        free(ptr); // free previous element
        ptr = tmp; // pass next element to ptr
    }
}