...
do
   ...
   echo -e "\tVM OS 1) sles11-sp3-x86_64, 2)sles11-sp3-i586, 3)sles12, 4)Win Server 2012): \c"
	read VMVARIANT	
   echo -e "\tVM Name:\c"
	read VMNAME
	echo -e "\tVM Mem(MB):\c"
   ...
   # sles11-sp3-x86_64
	 1)  
      OSVARIANT="sles11"
			...
			virt-install --virt-type ${VMHYPER} --$VMTYPE --name ${VMNAME} --memory ${VMMEM} --vcpus=${VMCPU} --disk ${VMDISK} --os-variant ${OSVARIANT} --network=bridge=br0 --location=nfs://"${TCIP}"/home/InstSource/SLES-11-SP3-DVD-x86_64-GM-DVD1/ --extra-args=autoyast=nfs://"${TCIP}"/opt/suse/testKits/system/autoyast/SLES11_SP3-x86_64-single_disk-GRUB.xml --graphics vnc &
	 ;;
   # sles11-sp3-i586  
   2)
      ARCH= "32" 
   ...
   3)
      OSVARIANT="sles12"
		virt-install --virt-type ${VMHYPER} --$VMTYPE --name ${VMNAME} --memory ${VMMEM} --vcpus=${VMCPU} --disk ${VMDISK} --os-variant ${OSVARIANT} --network=bridge=br0 --location=nfs://"${TCIP}"/home/InstSource/SLE-12-Server-DVD-x86_64-GM-DVD1/ --extra-args=autoyast=nfs://"${TCIP}"/opt/suse/testKits/system/autoyast/SLE_12-Server-x86_64-single_disk-GRUB.xml --graphics vnc  &
   ...
   # Windows Server 2012
	4)
		...
		read ISOPATH
		OSVARIANT="winserver2012"
		virt-install --virt-type ${VMHYPER} --$VMTYPE --name ${VMNAME} --memory ${VMMEM} --vcpus=${VMCPU} --disk ${VMDISK} --os-variant ${OSVARIANT} --network=bridge=br1 --cdrom=${ISOPATH} --graphics vnc  &
