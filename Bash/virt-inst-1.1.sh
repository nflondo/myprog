...
do
	...
	case "$VMVARIANT" in
	 ...
	 sles12)
		if [ "$VMHYPER" == "kvm" ]
		then
			virt-install --virt-type $VMHYPER --name $VMNAME --memory $VMMEM --vcpus=$VMCPU --disk $VMDISK --os-variant $VMVARIANT --network=bridge=br0 --location=nfs://"$TCIP"/home/InstSource/SLE-12-Server-DVD-x86_64-GM-DVD1/ --extra-args=autoyast=nfs://"$TCIP"/opt/suse/sles12-vm-autoinst.xml --graphics vnc &
	    else
			virt-install --virt-type $VMHYPER --$VMTYPE --name $VMNAME --memory $VMMEM --vcpus=$VMCPU --disk $VMDISK --os-variant $VMVARIANT --network=bridge=br0 --location=nfs://"$TCIP"/home/InstSource/SLE-12-Server-DVD-x86_64-GM-DVD1/ --extra-args=autoyast=nfs://"$TCIP"/opt/suse/sles12-vm-autoinst.xml --graphics vnc &
		fi
		;;
	win2k12)
		if [ "$VMHYPER" == "kvm" ]
